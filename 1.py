...
# Initialize video capture and set the frame width and height
...
# Define the keyboard layout with a backspace key
...
for i in range(len(keys)):
    for j, key in enumerate(keys[i]):
        if key == "Backspace":
            buttonList.append(Button([100 * j + 50, 100 * i + 50], key, [170, 85]))  # Wider size for Backspace
        else:
